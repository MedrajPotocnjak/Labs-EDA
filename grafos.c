...
void rellenarMatrizMaxGrado(Grafo* grafo, int* arreglo, int largo);
...
void armarGrafo(Grafo* grafo){
	...
	rellenarMatrizMaxGrado(grafo, grafoMaximo, maxMasUno);
	...
	if(min > 1 && aristasRestantes != 0){
		...
		//rellenarMatrizMinGrado(grafo, grafoMinimo, minMasUno);
...
void rellenarMatrizMaxGrado(Grafo* grafo, int* arreglo, int largo){
	...
	for(posicion = 0; posicion < nuevoLargo; posicion++){
		...
		grafo->matrizAdy[fila][columna] = 1;
		grafo->matrizAdy[columna][fila] = 1;
	}
}
//
